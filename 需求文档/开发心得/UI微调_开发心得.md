...
### 8. 灰玻璃 ≠ 提亮：灰要靠「中性灰雾」，水滴感要靠「边缘光学」
- **现象**：迷你条（及所有玻璃面）在深色模式下读作黑块，不像 Apple Music 那样的灰玻璃。
- **根因**：`GlassOverlay` 深色模式白雾透明度只有 0.07–0.12，叠在纯黑背景上有效亮度仅 ~#1A，低于人眼「灰色」感知阈值。
- **第一版弯路**：用「提高白雾透明度」（white@0.17+）补灰——纯黑底上亮度对了，但白雾叠在彩色内容上会把颜色**冲淡向白**，整体发奶白发亮，用户反馈「不是要更亮，是要更灰」。
- **正解**：雾基色换成**中性灰**（`AppTokens.glassFog` = iOS systemGray #8E8E93，浅/深共用同一基色，亮度交给透明度合成）。灰色雾把背景**压缩向灰**：亮处压暗、暗处提亮，像隔着水看，通透且不发白；黑底上 ~#2D 柔灰，白底上 ~#E1 浅灰，一次标定两模式都对。
- **水滴感的真正来源是边缘，不是整体**：受光亮线（左上）→ 快速衰减 → 厚度暗环 → **底部焦散亮线**（光穿过水滴聚焦在下缘）收进 rim 五段渐变描边；整体只留窄幅 specular（stops 0–0.14），不糊成白雾。整体色调越「平」，边缘光学越出挑。
- **教训**：透明叠加层的合成色要按目标背景反推 α（`背景×(1-α)+雾色×α`）；「灰」用带灰相的雾色去压，而不是用白色去抬；材质的「像水」主要由边缘折射/焦散特征决定，整体亮度只会破坏它。
